...
## 5.1 MCP prompt 与目录式 Skill

MCP 和 Skill 是两个独立扩展面。MCP manager 负责远程 tool、resource、连接、认证和 capability snapshot；`agent.with_skill(dir)` 只加载本地目录中的 `SKILL.md`。框架不会把远程 prompt 隐式注册成 Skill。

如果产品需要把 MCP prompt 包装成 Skill，应由应用先获取 prompt，再生成受审计的本地 Skill 目录后显式调用 `with_skill`。这样远程内容不会绕过 Skill 的文件来源和生命周期边界。
