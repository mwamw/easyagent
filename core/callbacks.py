...
class BaseCallback(ABC):
    ...
    def on_handoff(self, from_agent: str, to_agent: str, task: str, **kwargs) -> None:
        """
        Agent 间任务交接时 (Multi-Agent 编排器触发)
    ...
    def on_orchestrator_end(self, orchestrator_name: str, result: str, rounds: int, **kwargs) -> None:
        """
        编排器执行结束时 (Multi-Agent 编排器触发)
        
        Args:
            orchestrator_name: 编排器名称
            result: 最终输出
            rounds: 总轮次
        """
        pass
...
class CallbackManager:
    ...
    def on_handoff(self, from_agent: str, to_agent: str, task: str, **kwargs) -> None:
        for cb in self.callbacks:
            try:
                cb.on_handoff(from_agent, to_agent, task, **kwargs)
            except Exception as e:
                logger.warning(f"回调执行失败: {e}")
    ...
    def on_orchestrator_end(self, orchestrator_name: str, result: str, rounds: int, **kwargs) -> None:
        for cb in self.callbacks:
            try:
                cb.on_orchestrator_end(orchestrator_name, result, rounds, **kwargs)
            except Exception as e:
                logger.warning(f"回调执行失败: {e}")
